	...
	def updateScoreStrings( self ):
		...
		if ((CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY)):
			if (CyInterface().isScoresVisible() and not CyInterface().isCityScreenUp() and CyEngine().isGlobeviewUp() == false):
							...
							if (not CyInterface().isScoresMinimized() or gc.getGame().getActivePlayer() == ePlayer):
								if (gc.getPlayer(ePlayer).isAlive() and not gc.getPlayer(ePlayer).isMinorCiv()):
									...
									if (gc.getPlayer(ePlayer).getTeam() == eTeam):
													...
													for iPlayer in MetPlayerList:
														if (iPlayer != ePlayer and pTeam.isHasMet(gc.getPlayer(iPlayer).getTeam())):
															...
															if (pPlayer.getTradeDenial(iActivePlayer, tradeData) == DenialTypes.DENIAL_TOO_MANY_WARS):
																szWarPlan = u"(" + unichr(CyGame().getSymbolID(FontSymbols.OCCUPATION_CHAR)) + u")"
																break
												szTempBuffer = u"%d%s: <color=%d,%d,%d,%d>%s</color>" %(gc.getGame().getPlayerScore(ePlayer),szWarPlan, gc.getPlayer(ePlayer).getPlayerTextColorR(), gc.getPlayer(ePlayer).getPlayerTextColorG(), gc.getPlayer(ePlayer).getPlayerTextColorB(), gc.getPlayer(ePlayer).getPlayerTextColorA(), gc.getPlayer(ePlayer).getName())
										...
										screen.setText( szName, "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, xResolution - 12 - toho_width, yCoord - (iCount * iBtnHeight), -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_CONTACT_CIV, ePlayer, -1 )
										screen.show( szName )
				...
				screen.setPanelSize( "ScoreBackground", xResolution - 21 - iWidth - toho_width, yCoord - (iBtnHeight * iCount) - 4, iWidth + 12, (iBtnHeight * iCount) + 8 )
