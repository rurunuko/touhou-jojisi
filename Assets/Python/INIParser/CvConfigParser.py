...
class CvConfigParser(compat.SafeConfigParser, object):
	...
	def __init__(self, filename = None, *args, **kwargs):
		...
		if (filename != None):
			filenames = [os.path.join(os.path.dirname(dir), filename) for dir in CvPath.assetsPath]
			filenames.reverse()
			self.INIfilename = self.read(filenames)
			#CyInterface().addImmediateMessage("ConfigParser Read","")
			#for filename in self.INIfilename:
			#	CvUtil.pyPrint("Config Path: %s"%(filename))
	...
	def _wrappedGet(self, getter, section, option, default, *args, **kwargs):
		...
		except (compat.NoSectionError, compat.NoOptionError):
			if (default != None):
				return default
			else:
				raise
