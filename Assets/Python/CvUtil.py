...
#-------------------------
# Trade Resource Panel Option
EventTRPOption = 5100
# Playability Utilties
EventSignPopup = 5022
EventSitePopup = 5023
EventSpyMoveToCity = 5150
EventTestWarning = 5200
...
SilentEvents = [EventEditCityName, EventEditUnitName, EventReminderStore, EventReminderRecall, EventTRPOption, EventSitePopup, EventSignPopup, EventSpyMoveToCity]
...
#	combatMessage = "%s's %s (%.2f)" %(gc.getPlayer(cdAttacker.eOwner).getName(),cdAttacker.sUnitName,cdAttacker.iCurrCombatStr/100.0,)
	combatMessage = "%s%s %s (%.2f)" %(gc.getPlayer(cdAttacker.eOwner).getName(),localText.getText("TXT_KEY_APOSTROPHE_S", ()),cdAttacker.sUnitName,cdAttacker.iCurrCombatStr/100.0,)
# <<< CYBERFRONT
...
#	combatMessage += "%s's %s (%.2f)" %(gc.getPlayer(cdDefender.eOwner).getName(),cdDefender.sUnitName,cdDefender.iCurrCombatStr/100.0,)
	combatMessage += "%s%s %s (%.2f)" %(gc.getPlayer(cdDefender.eOwner).getName(),localText.getText("TXT_KEY_APOSTROPHE_S", ()),cdDefender.sUnitName,cdDefender.iCurrCombatStr/100.0,)
# <<< CYBERFRONT
