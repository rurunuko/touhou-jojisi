...
######## Tea Time (England(Koumakan)event) ###########

def canTriggerKoumakan(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
...
######## Doze Guard (England(Koumakan)event) ###########

def applyDozeGuard(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	iFlanking = CvUtil.findInfoTypeNum(gc.getPromotionInfo,gc.getNumPromotionInfos(),'PROMOTION_FLANKING1')
	
	unit.setHasPromotion(iFlanking, true)
	
def getHelpDozeGuard(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	szHelp = localText.getText("TXT_KEY_EVENT_CHAMPION_HELP", (unit.getNameKey(), gc.getPromotionInfo(iFlanking).getTextKey()))	

	return szHelp

def applyDozeGuard1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	iFlanking1 = CvUtil.findInfoTypeNum(gc.getPromotionInfo,gc.getNumPromotionInfos(),'PROMOTION_FLANKING2')

	unit.setHasPromotion(iFlanking1, true)
	
def getHelpDozeGuard1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	szHelp = localText.getText("TXT_KEY_EVENT_CHAMPION_HELP", (unit.getNameKey(), gc.getPromotionInfo(iFlanking1).getTextKey()))	

	return szHelp


######## A Good Cook(England(Koumakan)event) ###########

def canTriggerKoumakan1(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
...
def getHelpAGoodCook(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	
	szHelp = localText.getText("TXT_KEY_EVENT_BEST_DEFENSE_DONE_HELP_2", (2, ))	

	return szHelp

def applyAGoodCook(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	for iPlayer in range(gc.getMAX_CIV_PLAYERS()):			
		...
		if loopPlayer.isAlive() and iPlayer != kTriggeredData.ePlayer:
			loopTeam = gc.getTeam(loopPlayer.getTeam())
			if loopTeam.isHasMet(gc.getPlayer(kTriggeredData.ePlayer).getTeam()):
				loopPlayer.AI_changeAttitudeExtra(kTriggeredData.ePlayer, 2)

######## Prism Live(France(Hakugyokurou)event) ###########

def canTriggerHakugyokurou(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
...
######## Strategies Of Nine Tailed(France(Hakugyokurou)event) ###########

def canTriggerHakugyokurou1(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
...
######## Five Problems(Egypt(Eientei)event) ###########

def canTriggerEientei(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
...
######## Rice (Spain(Youkainoyama)event) ###########

def canTriggerYoukai(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)

	if (player.getCivilizationType() == gc.getInfoTypeForString("CIVILIZATION_SPAIN")):
		return true
	return false

def doRice2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
...
def getHelpRice2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	
	iFarm = CvUtil.findInfoTypeNum(gc.getImprovementInfo,gc.getNumImprovementInfos(),'IMPROVEMENT_FARM')
	szHelp = localText.getText("TXT_KEY_EVENT_IMPROVEMENT_GROWTH", ( gc.getImprovementInfo(iFarm).getTextKey(), ))

	return szHelp

######## Hot Spring(Persia(Chireiden)event) ###########

def canTriggerChireiden(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	
	if (player.getCivilizationType() == gc.getInfoTypeForString("CIVILIZATION_PERSIA")):
		return true
	return false

######## The Third Eyes(Persia(Chireiden)event) ###########

def canTriggerTheThirdEyesCity(argsList):
	iCity = argsList[2]
	
	player = gc.getPlayer(argsList[1])
	city = player.getCity(iCity)

	if not city.isHasBuilding(gc.getInfoTypeForString("BUILDING_SCOTLAND_YARD")):
		if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_COURTHOUSE")):
			if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_JAIL")):
				if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_NATIONAL_SECURITY")):
					if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_INTELLIGENCE_AGENCY")):
						if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_SATORIHOUSE")):
							return true
	return false

######## Hunch(Rome(Hyouseirengou)event) ###########

def canTriggerHyousei(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
...
######## Culture Of Supremacy(Rome(Hyouseirengou)event) ###########

def canTriggerCultureOfSupremacyCity(argsList):
	iCity = argsList[2]
	
	player = gc.getPlayer(argsList[1])
	city = player.getCity(iCity)

	if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_SISTINE_CHAPEL")):
		if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_HERMITAGE")):
			return true
	return false

######## Gramary Refinement(Japan(Hakureijinja)event) ###########

def canTriggerGramaryRefinement(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
	
	if (player.getCivilizationType() == gc.getInfoTypeForString("CIVILIZATION_JAPAN")):
		if team.getAtWarCount(true) <= 0:
			return true
	return false

######## Wonderful Shrine Maiden In Paradise(Japan(Hakureijinja)event) ###########

def canTriggerWonderfulShrineMaidenInParadise(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
...
def canTriggerWonderfulShrineMaidenInParadiseUnit(argsList):
	eTrigger = argsList[0]
	ePlayer = argsList[1]
	iUnit = argsList[2]
	...
	iZiki = CvUtil.findInfoTypeNum(gc.getPromotionInfo,gc.getNumPromotionInfos(),'PROMOTION_ZIKI')
	if unit.isHasPromotion(iZiki):
		return false

	return true
	
def applyWonderfulShrineMaidenInParadise(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	unit.setHasPromotion(iZiki, true)
	
def getHelpWonderfulShrineMaidenInParadise(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	szHelp = localText.getText("TXT_KEY_EVENT_CHAMPION_HELP", (unit.getNameKey(), gc.getPromotionInfo(iZiki).getTextKey()))	

	return szHelp

######## Knowledgeable(Japan(Hakureijinja)event) ###########

def canTriggerHakureijinja(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
...
######## Milking(India(Ningennosato)event) ###########

def canTriggerNingennosato(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	
	if (player.getCivilizationType() == gc.getInfoTypeForString("CIVILIZATION_INDIA")):
		return true
	return false

######## Drawn Pirate(Russia(Seirensen)event) ###########

def canTriggerSeirensen(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
...
def canTriggerDrawnPirateCity(argsList):
	eTrigger = argsList[0]
	ePlayer = argsList[1]
	iCity = argsList[2]
...
######## Treasure Search(Russia(Seirensen)event) ###########

def canTriggerTreasureSearchCity(argsList):
	iCity = argsList[2]
	
	player = gc.getPlayer(argsList[1])
	city = player.getCity(iCity)

	if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_ACADEMY")):
		return false
	if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_SCOTLAND_YARD")):
		return false
	if city.isHasBuilding(gc.getInfoTypeForString("BUILDING_MILITARY_ACADEMY")):
		return false
	return true
...
def getHelpSyoutokuMoney(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
...
def applySyoutokuMoney(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
...
#å≈óLéuå¸ä÷òAèàóù
#Ç±Ç±ÇÎÇøÇ·ÇÒéuå¸ïœâª

def canTraitKokoro(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())

	if player.hasTrait(gc.getInfoTypeForString('TRAIT_KOKOROLIST')):
		return true
	return false

def doTraitKokoro1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_KOKOROLIST')):
		...
		Traits = [ 'TRAIT_FINANCIAL','TRAIT_ORGANIZED','TRAIT_PHILOSOPHICAL','TRAIT_SPIRITUAL','TRAIT_ALCHOL' ]
		iRnd1 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_1")
		iRnd2 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_2")
		while iRnd2 == iRnd1:
			iRnd2 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_3")
...
def doTraitKokoro2(argsList):
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_KOKOROLIST')):
		for i in xrange(gc.getNumTraitInfos()):	
			pPlayer.setHasTrait(i,False)
		Traits = [ 'TRAIT_AGGRESSIVE','TRAIT_CHARISMATIC','TRAIT_PROTECTIVE','TRAIT_IMPERIALIST','TRAIT_SPELLIST','TRAIT_CENTRALIZATION' ]
		iRnd1 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_1")
		iRnd2 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_2")
		while iRnd2 == iRnd1:
			iRnd2 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_3")
...
def doTraitKokoro3(argsList):
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_KOKOROLIST')):
		for i in xrange(gc.getNumTraitInfos()):	
			pPlayer.setHasTrait(i,False)
		Traits = [ 'TRAIT_CREATIVE','TRAIT_EXPANSIVE','TRAIT_INDUSTRIOUS','TRAIT_TRADELIST','TRAIT_LABORLIST' ]
		iRnd1 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_1")
		iRnd2 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_2")
		while iRnd2 == iRnd1:
			iRnd2 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_3")
...
def doTraitKokoro4(argsList):
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_KOKOROLIST')):
		for i in xrange(gc.getNumTraitInfos()):	
			pPlayer.setHasTrait(i,False)
		Traits = [ 'TRAIT_AGGRESSIVE','TRAIT_CREATIVE','TRAIT_EXPANSIVE','TRAIT_FINANCIAL','TRAIT_INDUSTRIOUS','TRAIT_ORGANIZED','TRAIT_PHILOSOPHICAL','TRAIT_CHARISMATIC','TRAIT_PROTECTIVE','TRAIT_SPIRITUAL','TRAIT_IMPERIALIST','TRAIT_TRADELIST','TRAIT_ALCHOL','TRAIT_LABORLIST','TRAIT_SPELLIST','TRAIT_CENTRALIZATION','TRAIT_RAIDERS','TRAIT_UTOPIA','TRAIT_RESEARCH' ]
		iRnd1 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_1")
		iRnd2 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_2")
		while iRnd2 == iRnd1:
			iRnd2 = CyGame().getSorenRandNum(len(Traits), "KOKOROLIST_3")
...
def doTraitKokoro5(argsList):
	...
	pPlayer = gc.getPlayer(iPlayer)
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_KOKOROLIST')):
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_KOKOROLIST'),False)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_KOKOROLIST2'),True)

def getHelpKokoro1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_TRAIT_KOKOROLIST_1_HELP", ())

	return szHelp

def getHelpKokoro2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_TRAIT_KOKOROLIST_2_HELP", ())

	return szHelp

def getHelpKokoro3(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_TRAIT_KOKOROLIST_3_HELP", ())

	return szHelp

def getHelpKokoro4(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_TRAIT_KOKOROLIST_4_HELP", ())

	return szHelp

def getHelpKokoro5(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_TRAIT_KOKOROLIST_5_HELP", ())

	return szHelp

#ÉTÉOÉÅÇÃãtì]

def canTriggerGyakuten(argsList):	
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	otherPlayer = gc.getPlayer(kTriggeredData.eOtherPlayer)
	team = gc.getTeam(player.getTeam())
	
	if player.hasTrait(gc.getInfoTypeForString('TRAIT_SAGUMELIST')):
		if (team.AI_getAtWarCounter(otherPlayer.getTeam()) == 1):
			(loopUnit, iter) = otherPlayer.firstUnit(false)
			while( loopUnit ):
				plot = loopUnit.plot()
...
def doGyakuten1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
	iGoldenAgeTurn = player.getGoldenAgeLength()*3/2
	player.changeGoldenAgeTurns(iGoldenAgeTurn)

def getHelpGyakuten1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	iGoldenAgeTurn = player.getGoldenAgeLength()*3/2
	
	szHelp = localText.getText("TXT_KEY_EVENT_GYAKUTEN_1_HELP", (iGoldenAgeTurn, ))

	return szHelp

def doGyakuten2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)	
	capital = player.getCapitalCity()
	plot = gc.getMap().plot(kTriggeredData.iPlotX, kTriggeredData.iPlotY)
	iAge = player.getCurrentEra()
	iNumUnits = ((iAge+2)/2) * 6
		
	if None != capital and not capital.isNone():
		for i in range(iNumUnits):
			player.initUnit(capital.getConscriptUnit(), capital.getX(), capital.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)

def getHelpGyakuten2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	capital = player.getCapitalCity()
	...
	if None != capital and not capital.isNone():
		...
		szHelp = localText.getText("TXT_KEY_EVENT_GYAKUTEN_2_HELP", (iNumUnits, szUnit, capital.getNameKey()))

	return szHelp

#ÉwÉJTéuå¸ïœâª
def canThreeFace1(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())

	if player.hasTrait(gc.getInfoTypeForString('TRAIT_HECATIALIST')):
		if player.getCurrentEra() == gc.getInfoTypeForString('ERA_MEDIEVAL') or player.getCurrentEra() == gc.getInfoTypeForString('ERA_RENAISSANCE'):
			return true
	return false

def canThreeFace2(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())

	if player.hasTrait(gc.getInfoTypeForString('TRAIT_HECATIALIST')):
		if player.getCurrentEra() == gc.getInfoTypeForString('ERA_INDUSTRIAL') or player.getCurrentEra() == gc.getInfoTypeForString('ERA_MODERN') or player.getCurrentEra() == gc.getInfoTypeForString('ERA_FUTURE'):
			return true
	return false

def doThreeFace1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_HECATIALIST')):
		for i in xrange(gc.getNumTraitInfos()):	
			pPlayer.setHasTrait(i,False)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_PHILOSOPHICAL'),True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_SPELLIST'),True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_HECATIALIST'),True)

def doThreeFace2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
...
#å≈óLéuå¸ä÷òAèàóùÇ±Ç±Ç‹Ç≈

##ãPêjèÈå≈óLÉCÉxÉìÉgä÷òA
#îæãtÇÃìπãÔ

def canItemsResistance(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
	
	if (player.getCivilizationType() == gc.getInfoTypeForString("CIVILIZATION_MALI")):
		if team.getAtWarCount(true) <= 0:
			#éûë„îªíËÅBëæå√orå√ë„ÇÃéûÇÕãñâ¬ÇµÇ»Ç¢ÇÊÇ§Ç…
			if not (player.getCurrentEra() == gc.getInfoTypeForString('ERA_ANCIENT')) or (player.getCurrentEra() == gc.getInfoTypeForString('ERA_CLASSICAL')):
				return true
	return false

def getHelpItemsResistance2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_ITEMS_RESISTANCE_2_HELP", ())

	return szHelp

def getHelpItemsResistance3(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_ITEMS_RESISTANCE_3_HELP", ())

	return szHelp

#àÍê°ñ@étÇÃîÈïÛ
def canIssunsTreasure(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
...
def canIssunsTreasure1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	map = gc.getMap()
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	
	iUnita = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_SHINMYOUMARU0')
	iUnitb = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_SHINMYOUMARU1')
	iUnitc = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_SHINMYOUMARU2')
	iUnitd = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_SHINMYOUMARU3')
	iUnite = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_SHINMYOUMARU4')
	iUnitf = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_SHINMYOUMARU5')
	iUnitg = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_SHINMYOUMARU6')
	
	pPlayerA = pPlayer.getUnitClassCount(iUnita)
	pPlayerB = pPlayer.getUnitClassCount(iUnitb)
	pPlayerC = pPlayer.getUnitClassCount(iUnitc)
	pPlayerD = pPlayer.getUnitClassCount(iUnitd)
	pPlayerE = pPlayer.getUnitClassCount(iUnite)
	pPlayerF = pPlayer.getUnitClassCount(iUnitf)
	pPlayerG = pPlayer.getUnitClassCount(iUnitg)
	
	pAggregate = pPlayerA + pPlayerB + pPlayerC + pPlayerD + pPlayerE + pPlayerF + pPlayerG
...
def getHelpIssunsTreasure1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_ISSUNS_TREASURE_1_HELP", ())

	return szHelp

def getHelpIssunsTreasure2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_ISSUNS_TREASURE_2_HELP", ())

	return szHelp

def canIssunsTreasure3(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
	if (player.getLeaderType() == gc.getInfoTypeForString('LEADER_YATUHASHI')) or (player.getLeaderType() == gc.getInfoTypeForString('LEADER_BENBEN')):
		return true
	return false

def getHelpIssunsTreasure3(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_ISSUNS_TREASURE_3_HELP", ())

	return szHelp

#ïtërê_ÉâÉCÉu
def canLiveOfTsukumogami(argsList):
	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
...
def canLiveOfTsukumogami1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
	if (player.getLeaderType() == gc.getInfoTypeForString('LEADER_YATUHASHI')):
		return true
	return false

def getHelpLiveOfTsukumogami1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_LIVE_OF_TSUKUMOGAMI_1_HELP", ())

	return szHelp

def canLiveOfTsukumogami2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
	if (player.getLeaderType() == gc.getInfoTypeForString('LEADER_BENBEN')):
		return true
	return false

def getHelpLiveOfTsukumogami2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_LIVE_OF_TSUKUMOGAMI_2_HELP", ())

	return szHelp

def canLiveOfTsukumogami3(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
	if (player.getLeaderType() == gc.getInfoTypeForString('LEADER_RAIKO')):
		return true
	return false

def doLiveOfTsukumogami3(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	team = gc.getTeam(player.getTeam())
	player.changeGoldenAgeTurns(player.getGoldenAgeLength()/2)

def getHelpLiveOfTsukumogami3(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_LIVE_OF_TSUKUMOGAMI_3_HELP", ())

	return szHelp

##ãPêjèÈå≈óLÉCÉxÉìÉgä÷òAÇ±Ç±Ç‹Ç≈