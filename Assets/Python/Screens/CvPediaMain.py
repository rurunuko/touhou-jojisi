...
class CvPediaMain( CvPediaScreen.CvPediaScreen ):
	...
	def __init__(self):
		...
		##### <written by F> #####
		self.pediaTohoUnitScreen = CvPediaTohoUnit.CvPediaTohoUnit(self)
		##### </written by F> #####
		...
		self.mapCategories = { 
			...
			##### <written by F> #####
			CivilopediaPageTypes.CIVILOPEDIA_PAGE_TOHOUNIT	: self.placeTohoUnits, 
			##### </written by F> #####
	...
	def setPediaCommonWidgets(self):
		...
		##### <written by F> #####
		self.szCategoryTohoUnit = localText.getText("TXT_KEY_PEDIA_CATEGORY_TOHOUNIT", ())
		##### </written by F> #####
	...
	def placeTohoUnits(self):
		...
		if gc.getDefineINT("CIVILOPEDIA_SHOW_ACTIVE_CIVS_ONLY") and gc.getGame().isFinalInitialized():
			...
			for item in listCopy:
				if not gc.getGame().isUnitEverActive(item[1]):
					list.remove(item)
				elif gc.getUnitInfo(item[1]).getUnitCombatType() != gc.getInfoTypeForString('UNITCOMBAT_STANDBY'):
					list.remove(item)
		...
		for item in list:
			...
			if self.iActivePlayer != -1:
				szButton = gc.getPlayer(self.iActivePlayer).getUnitButton(item[1])
			screen.setTableText(tableName, iColumn, iRow, u"<font=3>" + item[0] + u"</font>", szButton, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TOHOUNIT, item[1], 1, CvUtil.FONT_LEFT_JUSTIFY)
			iCounter += 1
	...
	def pediaJump(self, iScreen, iEntry, bRemoveFwdList):
		...
		#CvGameUtils.doprint("test:%d %d %d" %(iScreen,CvScreenEnums.PEDIA_UNIT,CvScreenEnums.PEDIA_TOHOUNIT))
	...
	def handleInput (self, inputClass):
		...
		#CvGameUtils.doprint(str(inputClass))
