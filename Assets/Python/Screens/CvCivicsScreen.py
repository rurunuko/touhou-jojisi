...
# VET DynamicSize - begin 2/7
		...
		for i in range(gc.getNumCivicInfos()):
			aiCivicOptionNum[gc.getCivicInfo(i).getCivicOptionType()] += 2 #(2 * self.TEXT_MARGIN)
		iMax = 0
		for i in range(gc.getNumCivicOptionInfos()):
		...
		for i in range(gc.getNumCivicInfos()):
			...
			if iLines > iMax:
				iMax = iLines
		iHELP_HEIGHT = iMax * 22 + 4 * self.TEXT_MARGIN + 2
