...
class IconGrid:
	...
	def __init__(self, sWidgetId, screen, iX, iY, iWidth, iHeight, columns, bUseSmallIcons, bShowRowHeader, bShowRowBorder):
		...
		self.scrollSpace = 3
		self.iSmallIconSize = 32
		self.iSmallIconColWidth = self.iconColWidth + 4
		self.iSmallIconOffset = (64 - self.iSmallIconSize)/2
	...
	def getPrefferedWidth(self):
		...
		for index in xrange(len(self.columns)):
			...
			elif (self.columns[index] == GRID_SMALL_ICON_COLUMN):
				prefferedWidth += self.iSmallIconColWidth
 		...
 		for rowIndex in xrange(maxIndex):
			...
			for groupIndex in xrange(len(self.columnGroups)):
				...
				for offset in xrange(colGroup.length):
					...
					elif (self.columns[startIndex + offset] == GRID_SMALL_ICON_COLUMN):
						iconData = rowData.cells[startIndex + offset].icons[0]
						self.screen.setImageButton( self.rowName + str(rowIndex) + "_" + str(startIndex + offset)
												  , iconData.image, currentX + self.iSmallIconOffset, currentY + self.iSmallIconOffset, self.iSmallIconSize, self.iSmallIconSize
												  , iconData.widgetType, iconData.data, -1 )
						currentX += self.iSmallIconColWidth + self.colSpace
	...
	def calculateLayout(self):
		...
		for colIndex in xrange(len(self.columns)):
			...
			if (col == GRID_SMALL_ICON_COLUMN):
				availableWidth -= self.iSmallIconColWidth
			...
			if (colGroup.label != ""):
				...
				for offset in xrange(colGroup.length):
					if (self.columns[startIndex + offset] == GRID_ICON_COLUMN):
						...
						colGroupWidth += self.iSmallIconColWidth + self.colSpace
				...
				for offset in xrange(colGroup.length):
					if (self.columns[startIndex + offset] == GRID_ICON_COLUMN):
						...
						colGroupX += self.iSmallIconColWidth + self.colSpace
			startIndex += colGroup.length
		...
		for groupIndex in xrange(len(self.columnGroups)):
			...
			for offset in xrange(colGroup.length):
				if (self.columns[startIndex + offset] == GRID_ICON_COLUMN):
					...
					headerWidth = self.iSmallIconColWidth + self.colSpace
